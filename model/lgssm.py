...
from .misc import validate_shape
...
class StateSpaceModel(nn.Module):
    def __init__(
        ...
        self.a_0 = nn.Parameter(torch.zeros(config.a_dim))
        self.K = config.K

        self._mat_A_K = nn.Parameter(
            (1.0 - config.init_transition_reg_weight)
            * torch.randn(config.K, config.z_dim, config.z_dim)
            + config.init_transition_reg_weight * torch.eye(config.z_dim)
        )
        self._mat_C_K = nn.Parameter(
            (1.0 - config.init_observation_reg_weight)
            * torch.randn(config.K, config.a_dim, config.z_dim)
            + config.init_observation_reg_weight * torch.eye(config.a_dim, config.z_dim)
        )

        mat_Q = config.init_noise_scale * torch.eye(config.z_dim)
        mat_R = config.init_noise_scale * torch.eye(config.a_dim)
        ...
        self.learn_noise_covariance = config.learn_noise_covariance
        self._a_eye = torch.eye(config.a_dim)
        self._z_eye = torch.eye(config.z_dim)
        self.Q_reg = Q_reg
        self.R_reg = R_reg
        ...
        if initial_state_mean is None:
            self.initial_state_mean = torch.zeros(config.z_dim)
        else:
            validate_shape(initial_state_mean, (config.z_dim,), "initial_state_mean")
            self.initial_state_mean = initial_state_mean

        if initial_state_covariance is None:
            self.initial_state_covariance = torch.eye(config.z_dim)
        else:
            validate_shape(
        ...
        self.weight_model = LSTMModel(
            config.a_dim,
            config.K,
            hidden_dim=config.dpn_hidden_size,
            num_layers=config.dpn_num_layers,
        )
    ...
    def mat_Q(self, value):
        # shape: (z_dim, z_dim)
        validate_shape(value, (self.z_dim, self.z_dim), "mat_Q")
    ...
    def mat_R(self, value):
        # shape: (a_dim, a_dim)
        validate_shape(value, (self.a_dim, self.a_dim), "mat_R")
        self._mat_R_L = nn.Parameter(torch.linalg.cholesky(value))
        self.R_reg = 0.0

    @mat_A_K.setter
    def mat_A_K(self, value):
        validate_shape(value, (self.K, self.z_dim, self.z_dim), "mat_A_K")
        self._mat_A_K = nn.Parameter(value)

    @mat_C_K.setter
    def mat_C_K(self, value):
        validate_shape(value, (self.K, self.a_dim, self.z_dim), "mat_C_K")
